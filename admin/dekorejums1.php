<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_POST['id']) && !empty($_POST['id'])) {
            ...
            $sql = "UPDATE sparkly_dekorejums1 SET " . implode(", ", $sql_parts) . " WHERE id_dekorejums1 = ?";
            $stmt = $savienojums->prepare($sql);
            ...
            if (isset($_FILES['attels']) && $_FILES['attels']['error'] == 0) {
                ...
                $sql = "INSERT INTO sparkly_dekorejums1 (nosaukums, attels) VALUES (?, ?)";
                $stmt = $savienojums->prepare($sql);
        ...
        echo "<script>window.location.href = 'dekorejums1.php';</script>";
    }
...
<main>
    ...
    <section class="admin-content">
        <h1>Dekorējumu (1) pārvaldība</h1>
        
        <!-- Decorations Table -->
        <div class="product-table-container">
            <div class="table-header">
                <h2>Esošie dekorējumi (1)</h2>
                <a href="dekorejums1.php?action=add" class="btn add-btn"><i class="fas fa-plus"></i></a>
            </div>
                ...
                <table class="product-table">
                    ...
                    <tbody>     
                        <?php
                            ...
                            $sql = "SELECT * FROM sparkly_dekorejums1 ORDER BY id_dekorejums1";
                            $result = $savienojums->query($sql);
                            
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr>";
                                    echo "<td>{$row['id_dekorejums1']}</td>";
                                    ...
                                    echo "<a href='dekorejums1.php?edit={$row['id_dekorejums1']}' class='btn edit-btn'><i class='fas fa-edit'></i> Rediģēt</a>";
                                    echo "<a href='dekorejums1.php?delete={$row['id_dekorejums1']}' class='btn delete-btn' onclick='return confirm(\"Vai tiešām vēlaties dzēst šo dekorējumu?\")'><i class='fas fa-trash-alt'></i> Dzēst</a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                            } else {
        ...
        <div class="custom-form-container">
            ...
            <form class="custom-form" method="POST" enctype="multipart/form-data">
                <?php if ($editData): ?>
                    <input type="hidden" name="id" value="<?php echo $editData['id_dekorejums1']; ?>">
                <?php endif; ?>
                ...
                <div class="form-buttons">
                    ...
                    <a href="dekorejums1.php" class="btn clear-btn">Atcelt</a>
                </div>
            </form>
        </div>
        <?php endif; ?>
...
<main>
    ...
    <section class="admin-content">
        <h1>Dekorējumu (2) pārvaldība</h1>
        
        <!-- Decorations Table -->
        <div class="product-table-container">
            <div class="table-header">
                <h2>Esošie dekorējumi (2)</h2>
